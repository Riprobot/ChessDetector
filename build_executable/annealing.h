...
namespace annealing {
    ...
    float cross_value(const Image &img, int left, int right, int up, int down) {
        uint64_t hash_val = (uint64_t)left + 1000*(uint64_t)right + 1000000*(uint64_t)up + 1000000000*(uint64_t)down;
//        std::cout << "cross_value " << left << " " << right << " " << up << " " << down << std::endl;
//        std::cout << "hash_val=" << hash_val << std::endl;
        if (cross_value_map.count(hash_val)) {
            return cross_value_map[hash_val];
        }
...
//        std::cout << "A" << std::endl;
        cross_value_map[hash_val] = result;
        return result;
    }
    ...
    struct Board {
        Board(const Image &img, std::array<int, BOARD_SIZE> l, std::array<int, BOARD_SIZE> r,
              std::array<int, BOARD_SIZE> u, std::array<int, BOARD_SIZE> d) : left(l),
                                                                                  ...
                                                                                  down(d) {
            int edge_delta = 10;
//            std::cout << "### " << edge_delta << " " << IMG_SIZE - 1 - edge_delta << std::endl;
...
//                std::cout << "! " << left[i] << " " << right[i] << " " << up[i] << " " << down[i] << std::endl;
            }
...
//            std::cout << "cross_result=" << cross_result << " form_result=" <<  form_result << " result=" << result << std::endl;
            result += cross_result;
            result += form_result;
            score = result;
            worst_horizontal = 0;
...
//            if (i % 1000 == 0) {
//                std::cout << "i=" << i << " score=" << board.score << " " << "t=" << t << std::endl;
//            }
        }
        return board;
    }
